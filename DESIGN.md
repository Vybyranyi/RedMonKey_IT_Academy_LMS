...
2. `Enter` → зберегти й відкрити клітинку наступного студента в тому ж занятті (фокус одразу в полі — «10 Enter 8 Enter …»). `Tab` — так само, якщо оцінку змінили; по незміненій — на поле коментаря
3. `Escape` → скасувати, фокус повертається на клітинку
4. Hover → tooltip з деталями (хто/коли поставив); для скрінрідера клітинка називає студента, заняття й оцінку (`cellLabel`)

### Фокус і доступність

- Видимий фокус з клавіатури — базове `:focus-visible` в `index.css`: контур 2px кольору `--ring` з відступом 2px для всього, що не має власного кільця. ShadCN-примітиви мають своє (`outline-none` + `focus-visible:ring`), утиліти перекривають базовий шар
- На темних поверхнях (`#29425D`: Sidebar, Bottom Nav, меню «Ще») контур білий — додай контейнеру клас `focus-on-dark`. Червоний на темно-синьому майже не видно
- Не став `focus:outline-none` без заміни. Дії, що з'являються на hover (`opacity-0 group-hover:opacity-100`), — ще й `focus-visible:opacity-100`, інакше фокус з клавіатури потрапляє на невидиму кнопку
- Іконкова кнопка без тексту — завжди з `aria-label`; перемикачі — з `aria-pressed`
- Селект-фільтр без видимого підпису — з `<Label htmlFor className="sr-only">` і `id` на `SelectTrigger`. Не `aria-label`: на combobox він перекриє обране значення
