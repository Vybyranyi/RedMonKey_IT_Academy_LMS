...
- [ ] `NotFoundPage` + catch-all роут — зараз невідомий URL дає порожній екран
- [ ] Сторінка/стан 403 при відмові `ProtectedRoute` за роллю
- [ ] React `ErrorBoundary` навколо `AppLayout`
- [ ] Втрата сесії: провалений refresh → чистий логаут з toast, без циклу запитів
- [ ] Loading states + Skeleton UI
- [ ] Empty states (порожні списки)
- [ ] Оптимізація: кешування/скасування паралельних запитів (React Query у проєкті немає — стан на Zustand + axios)
- [ ] **Оптимістичні оновлення замість повного перемальовування.** Зараз після кожної мутації сторінка перезапитує весь список і показує скелетон — журнал оцінок, баланс монет і відвідуваність блимають цілком через зміну одного значення. Треба: одразу застосувати зміну до локального стану, відправити запит у фоні, відкотити стан і показати toast при помилці. Оновлювати точково — конкретний запис у масиві за `id`, а не замінювати весь масив відповіддю сервера.
  - [ ] `GradeCell` — оцінка з'являється в клітинці миттєво, решта журналу не перерендерюється
  - [ ] `CoinAwardForm` — баланс і історія транзакцій оновлюються локально після нарахування
  - [ ] `AttendanceList` — перемикання статусу студента не перезавантажує список
  - [ ] Мемоїзація рядків журналу (`React.memo` + стабільні `key`/колбеки), щоб зміна однієї клітинки не тягнула перерендер усієї таблиці
