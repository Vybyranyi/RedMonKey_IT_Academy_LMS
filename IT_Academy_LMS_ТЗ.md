...
`endDate` пізніше за `startDate` — і коли в `PATCH` приходить лише одна з дат (друга береться з БД).
...
Домашнє завдання — необов'язкові `homeworkDescription` і `homeworkDueDate` (ISO; `null` у `PATCH` прибирає дедлайн). Дедлайн не раніше за дату заняття, зокрема коли заняття переносять пізніше вже збереженого дедлайну.
...
2. Натиск Enter або Tab — зберегти і перейти до наступного студента в тому ж занятті (його попап відкривається одразу). Tab по незміненій оцінці веде на коментар
3. Escape — скасувати
4. Hover показує підказку з деталями оцінки (хто/коли); для скрінрідера клітинка називає студента, заняття й оцінку
...
- [x] Favicon + `<title>` по роутах — favicon з логотипа (раніше `index.html` посилався на неіснуючий `/favicon.svg`), вкладка «Розділ · IT Academy LMS» з того ж `getPageMeta`, що й заголовок у Header
- [x] A11y-мінімум: фокус-стани, `aria-label` на іконкових кнопках, перевірити клавіатурну навігацію журналу (Tab/Enter/Escape з розділу 6.4) — базове `:focus-visible` (білий на темних Sidebar і Bottom Nav), назви для кнопки пароля, «Вийти» (була невидима при фокусі) і селектів-фільтрів; у журналі Enter/Tab зберігають і відкривають клітинку наступного студента, Escape скасовує. Аудит axe-core по всіх сторінках — без порушень, крім розмітки react-big-calendar
